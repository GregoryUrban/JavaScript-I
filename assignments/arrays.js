...
let carYears = [];
  for(let i = 0; i < inventory.length; i++ ) {
    let currentCarYear = inventory[i].car_year; 
    carYears.push(currentCarYear)
  }
...
let BMWAndAudi =[];
  ...
  for(let i = 0; i < inventory.length; i++ ) {
    let carData = inventory[i]
    if (carData.car_make === 'BMW') {
      BMWAndAudi.push(carData);
    }

    if (carData.car_make === 'Audi') {
      BMWAndAudi.push(carData);
    }
